...
源码和 registry 两种模式都会再连接四个 Rust 身份，通过校验证书的 WSS 验证两个群。
可信验收工具通过 Product HTTP 建群、管理成员，SDK 客户端只连接消息网关。
十个场景覆盖成员投递、群间隔离、非成员拒绝（原因码 3）、添加/移除/重新加入、
黑名单拒绝（原因码 4）及解除，以及四个客户端断网自动重连后的成员权限。
...
验收覆盖前三个客户端之间全部六条有向单聊路径、群成员投递和频道隔离、非成员
及黑名单拒绝、成员移除与重新加入，以及接入节点 1 被强制终止、使用原地址和
原数据目录重启后的权限保持。阻断 SENDACK 时要求发送返回 `Timeout`，但对端
已收到该消息。独立出站计数必须等于全部应用 SEND 次数（含被拒绝和结果未知的
发送），检查重连前后没有重发，避免服务端去重掩盖重发。应用不重试未知结果的消息。

连接恢复与在线路由恢复分别验证。试运行在 Slot 权威变更后观察到接收者不在
在线路由列表，此时立即发送会出现有 SENDACK 而无投递。正式检查先要求从三个
API 入口连续两个 25 秒心跳周期都能查询到四个用户在线，再验证稳定状态的投递。
观察窗口 50 秒，整个路由检查最多等待 100 秒；回执区分连接恢复与观察完成时间。
SENDACK 仍只表示服务端接受，不表示接收者已收到。

[独立 CI](.github/workflows/cluster.yml) 分别对源码和精确正式包运行 60 秒工作负载，
手动运行可选择 600 秒。初始权限和故障检查先执行，计时工作负载中途再次终止、
重启接入节点并检查恢复。计时包含恢复时间且会完成当前阶段，因此短运行可能超过
60 秒。通过条件还包括所有客户端销毁、在线查询为空、自有进程和代理连接/任务
均为零。失败回执保留限量路由与投递诊断。测试配置为 SEND 3 秒、连接 2 秒、
PONG 10 秒、心跳 25 秒、重连最多 100 次、退避 100–500 ms，与 SDK 默认值不同。
每阶段至少观察被排除的客户端 500 ms。

此限时验收不代表其他地址自动切换、故障期间连续可用、指定 Channel Leader
切换、网络分区恢复、离线补拉、大群容量或多日稳定性已经验证。
